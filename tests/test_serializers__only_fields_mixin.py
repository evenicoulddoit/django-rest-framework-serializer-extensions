...
class OnlyFieldsSerializerMixinTests(SerializerMixinTestCase):
    ...
    def serialize(self, **context):
        ...
        """
        return CarModelTestSerializer(
            self.carmodel_model_s, context=context
        ).data
    ...
    def test_all_fields_explicit(self):
        self.assertDictEqual(
            self.serialize(only=set()), self.expected_complete_data
        )

    def test_only_single_root_field(self):
        self.assertDictEqual(
            self.serialize(only={"id"}), dict(id=self.carmodel_model_s.pk)
        )

    def test_only_multiple_root_fields(self):
        self.assertDictEqual(
            self.serialize(only={"id", "name"}),
            dict(id=self.carmodel_model_s.pk, name=self.carmodel_model_s.name),
        )
    ...
    def test_only_single_child_field_foreign_key(self):
        self.assertDictEqual(
            ...
            dict(manufacturer=dict(id=self.manufacturer_tesla.pk)),
        )
    ...
    def test_only_single_child_field_many(self):
        self.assertDictEqual(
            ...
            dict(skus=[dict(id=self.sku_p100d.pk), dict(id=self.sku_70.pk)]),
        )
    ...
    def test_only_multiple_child_fields_many(self):
        self.assertDictEqual(
            ...
            dict(
                skus=[
                    dict(id=self.sku_p100d.pk, variant=self.sku_p100d.variant),
    ...
    def test_double_nested_only(self):
        self.assertDictEqual(
            ...
            dict(
                skus=[
                    dict(owners=[dict(name=self.owner_tyrell.name)]),
                    dict(owners=[]),
                ]
            ),
        )
